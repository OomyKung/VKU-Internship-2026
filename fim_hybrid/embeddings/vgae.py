"""Variational graph autoencoder embeddings."""
...
class VGAEEmbedding(GraphEmbeddingModel):
    """Learn probabilistic node embeddings with a VGAE objective."""
    ...
    def _fit_impl(
        ...
        *,
        ...
        torch, nn_module, _, _, vgae_cls, _, gcn_conv_cls, _ = require_pyg_dependencies()
        ...
        encoder = build_variational_gcn_encoder(
            nn_module,
            gcn_conv_cls,
        ...
        for _ in range(epochs):
            ...
            loss = loss + (model.kl_loss() / max(graph_data.node_count, 1))
            loss.backward()
            optimizer.step()
