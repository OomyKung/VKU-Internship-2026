"""Shared optimizer utilities for Fair Influence Maximization."""
...
class EAOptimizerConfig:
    """Explicit configuration for the unified EA+Memetic optimizer."""
    ...
    # Large-group minimum quota โ€” groups whose node-fraction โฅ large_group_size_threshold
    # receive at least ceil(budget ร— size_ratio ร— large_group_min_quota_ratio) seeds
...
class EAOptimizerBase:
    """Simple unified hybrid optimizer combining removed guidance and EA operators."""
    ...
    def _group_overshoot_cap_met(self, group_name: str, group_counts: dict[str, int]) -> bool:
        """Return True when group_name has accumulated >= cap ร— proportional_seed_target seeds.
        ...
        for group_name, target in target_counts.items():
            if group_counts.get(group_name, 0) < int(target):
                if self._group_overshoot_cap_met(group_name, group_counts):
                    # Cap prevents this repair โ€” count as a redirected slot.
    ...
    def optimize(self) -> OptimizationResult:
        """Run the unified EA+Memetic optimizer end-to-end."""
        ...
        for generation in range(self.config.generations):
            ...
            reference_pool = [evaluation.seed_set for evaluation in elites]
            ...
            for evaluation in ranked_current:
                ...
                if (
                    ...
                    mate = reference_pool[int(self.rng.integers(len(reference_pool)))]
