...
## Fast Run Repeated Evaluation (fim_evaluations)

The table below collects all fast-mode runs saved under `results/fim_evaluations/`. All runs use the `twitter` dataset with protected attribute `color`, budget `120`. The `_fast` variant uses RR=512, MC=300, population=30, generations=20. The `_fast_mf_tuned` variant uses RR=768, MC=300 with additional MF-lift tuning.
